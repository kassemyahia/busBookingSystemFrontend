(async () => {
  ...
  try {
    ...
    root.innerHTML = `<section class="max-w-3xl rounded-2xl border bg-white p-6 shadow-sm"><div class="grid gap-5 sm:grid-cols-2">${fields.map(([label, value]) => `<div><p class="text-xs font-semibold uppercase tracking-wide text-slate-500">${admin.esc(label)}</p><p class="mt-1 font-semibold">${admin.esc(value)}</p></div>`).join("")}</div><p class="mt-6 rounded-xl bg-slate-50 p-4 text-sm text-slate-600">These details are read-only because the API does not expose an employee self-update endpoint.</p></section>`;
  } catch (error) {
    loading.classList.add("hidden");
    if (error.status === 401) {
