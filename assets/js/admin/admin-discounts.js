(() => {
  async function init(kind) {
    ...
    const trip = kind === "trip",
      base = trip ? "/api/admin/tripdiscounts" : "/api/admin/userdiscounts",
      active = trip ? "active-trip-discounts" : "active-user-discounts",
      deleted = trip ? "deleted-trip-discounts" : "deleted-user-discounts",
      idKey = trip
        ? ["tripDiscountId", "TripDiscountId", "id", "Id"]
        : ["userDiscountId", "UserDiscountId", "id", "Id"];
    async function load() {
    ...
    function form(title, id = "", name = "", pct = "") {
      admin.openModal(
        title,
        admin.input("Name", "Name", "text", name, 'required maxlength="50"') +
          admin.input(
            "Percentage",
            "Percentage",
            "number",
        ...
        async (f) => {
          ...
          if (id) {
            body = trip
              ? {
                  TripDiscountId: Number(id),
                  Name: f.get("Name"),
                  Percentage: Number(f.get("Percentage")),
                }
              : {
                  UserDiscountId: Number(id),
                  Name: f.get("Name"),
                  DiscountPercentage: Number(f.get("Percentage")),
                };
          } else
            body = {
              Name: f.get("Name"),
          ...
          await admin.request(`${base}/${id ? "update" : "add"}`, {
            method: id ? "PUT" : "POST",
    ...
    document.getElementById("addButton").onclick = () => form("Add discount");
    load();
  }
  window.discountPage = { init };
})();
