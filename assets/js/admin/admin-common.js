(() => {
  ...
  function openModal(title, body, onSubmit) {
    ...
    root.innerHTML = `<div class="fixed inset-0 z-50 grid place-items-center bg-slate-950/60 p-4 backdrop-blur-sm" role="dialog" aria-modal="true"><div class="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-6 shadow-2xl"><div class="mb-5 flex items-center justify-between"><h2 class="text-xl font-bold">${esc(title)}</h2><button data-close aria-label="Close dialog" class="rounded-lg px-3 py-2 text-slate-500 hover:bg-slate-100">✕</button></div><form id="modalForm" class="space-y-4">${body}<div id="modalError" class="hidden rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-700"></div><div class="flex justify-end gap-3"><button type="button" data-close class="rounded-xl border px-4 py-2">Cancel</button><button id="modalSubmit" class="rounded-xl bg-teal-600 px-4 py-2 font-semibold text-white hover:bg-teal-700">Save</button></div></form></div></div>`;
    root
